...
@section('header', 'Tambah Fakta Nusantara')
...
<div class="p-6 max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800">Tambah Fakta Baru</h1>
            <p class="text-sm text-slate-500">Fakta akan ditampilkan secara acak di halaman publik.</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <form action="{{ route('facts.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="content" rows="5" maxlength="1000"
                    ...
                    class="w-full rounded-xl border-gray-200 bg-white text-slate-900 focus:ring-primary focus:border-primary px-4 py-3 text-sm resize-none @error('content') border-red-400 @enderror">{{ old('content') }}</textarea>
                @error('content')
