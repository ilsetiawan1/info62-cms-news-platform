...
@section('content')
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-none border border-gray-100 dark:border-gray-700 p-6 hover:-translate-y-1 hover:shadow-lg transition-all duration-300 group">
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-slate-900 dark:text-gray-50">{{ number_format($totalArticles) }}</h3>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-none border border-gray-100 dark:border-gray-700 p-6 hover:-translate-y-1 hover:shadow-lg transition-all duration-300 group">
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-slate-900 dark:text-gray-50">{{ number_format($totalViews) }}</h3>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-none border border-gray-100 dark:border-gray-700 p-6 hover:-translate-y-1 hover:shadow-lg transition-all duration-300 group">
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-slate-900 dark:text-gray-50">{{ number_format($activeCategories) }}</h3>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-none border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
            ...
            <a href="{{ route('articles.index') }}" class="text-primary dark:text-primary-500 hover:text-primary/80 dark:hover:text-primary-500/80 text-sm font-medium">Lihat Semua</a>
        </div>
        <div>
            @forelse($recentActivities as $activity)
                <div class="px-6 py-4 border-b border-gray-50 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <div class="w-10 h-10 rounded-full bg-primary/10 dark:bg-primary-500/20 text-primary dark:text-primary-500 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-slate-900 dark:text-gray-100">
                                {{ $activity->author->name ?? 'Sistem' }} menambahkan artikel baru
                            </p>
                            <p class="text-sm text-slate-500 dark:text-gray-400 truncate max-w-md">
                                "{{ $activity->title }}" di kategori {{ $activity->category->name ?? 'Umum' }}
                            </p>
                        </div>
                    </div>
                    <div class="text-xs text-slate-400 dark:text-gray-500 whitespace-nowrap">
